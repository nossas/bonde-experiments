...
// import { updateFormEntries, generateMobilizations } from "./";

const FORM_ENTRIES_SUBSCRIPTION = gql`
subscription pipeline_form_entries($widgets: [Int!]) {
  form_entries(where: {mailchimp_syncronization_at: {_is_null: true}}, order_by: {created_at: asc}) {
    id
    fields
    cached_community_id
    activist_id
...
const handleNext = (widgets: Widget[]) => async (response: any) => {
  ...
  // Escola, Centro Educacional, Colégio, Instituto, Pré Vestibular, EJA
  ...
  if (cache.length > 0) {
    ...
    // await generateMobilizations(individuals);
...
export default async (widgets: Widget[]): Promise<any> => {
    ...
    console.error("failed on subscription: ", err);
    return undefined;
  }
};
